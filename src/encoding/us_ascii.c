...
static int8_t us_ascii_scan_mbc_width(const nk_encoding_t* enc ARG_UNUSED, const uint8_t* bytes,
...
static nk_error_t us_ascii_encode_mbc(const nk_encoding_t* enc ARG_UNUSED, uint32_t code, size_t* out_width,
                                      uint8_t* out_bytes) {
  if (code < 128) {
    *out_width = 1;
    if (out_bytes != NULL) {
