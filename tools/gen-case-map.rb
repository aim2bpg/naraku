...
def gen_ascii(case_map)
  ...
  codes.each_with_index do |code, index|
    ...
    if fold_result.length > 1
      raise "Unexpected fold result for ASCII code #{code}: #{fold_result.inspect}"
    end
  ...
  diff_map.each do |diff, range_set|
    ...
    range_set.each_range do |r|
      if r.begin == r.end
        cond << "code == 0x%02X" % r.begin
      else
        cond << ("(code >= 0x%02X && code <= 0x%02X)" % [r.begin, r.end])
      end
    end
    puts "  if (#{cond.join(' || ')}) {"
    puts "    unfold_items[0].folded_codes_len = 1;"
    puts "    unfold_items[0].unfolded_code = (uint32_t)(code #{diff < 0 ? '-' : '+'} #{diff.abs});"
    puts "    return 1;"
    puts "  }"
  end
  ...
  diff_map.each do |diff, range_set|
    range_set.each_range do |r|
      puts "  for (uint32_t code = 0x%02X; code <= 0x%02X; code++) {" % [r.begin + diff, r.end + diff]
      puts "    uint32_t folded_code = (uint32_t)(code #{diff < 0 ? '+' : '-'} #{diff.abs});"
...
GPERF_COMMAND = %w[gperf -L ANSI-C -c -j1 -i0 -t -C -P -T -E]
...
def gen_unicode(case_map)
  ...
  fold_items = case_map.fold_items.to_a.sort_by! { |from_code, item| from_code }
  fold_items.each_with_index do |(from_code, item), index|
    ...
    to_type_flags = []
    to_types.each do |to_type|
      to_type_flags << flags[to_type]
    end
  ...
  case_map_items = case_map.case_map_items.to_a.sort_by! { |from_code, item| from_code }
  case_map_items.each do |from_code, item|
    to_code = item[:to_codes].first
    if item[:to_codes].length > 1
      raise "Unexpected multiple to_codes for case map item with from_code 0x%04X: #{item[:to_codes].inspect}" % from_code
    end
    specials = item[:specials].dup

    type_flags = []
    item[:to_types].each do |type|
  ...
  fold_table_source = IO.popen([*GPERF_COMMAND, '-H', 'unicode_fold_table_hash', '-Q', 'unicode_fold_table_pool', '-N', 'unicode_fold_table_lookup', err: :err], 'w+') do |io|
    io.puts '%{'
    io.puts '%}'
    ...
    fold_items.each_with_index do |(from_code, _), index|
      ...
      io.puts "%-20s %d" % ["\"#{from_code_str}\",", index]
    end
    io.puts '%%'
    io.close_write
  ...
  case_map_table_source =  IO.popen([*GPERF_COMMAND, '-H', 'unicode_case_map_table_hash', '-Q', 'unicode_case_map_table_pool', '-N', 'unicode_case_map_table_lookup', err: :err], 'w+') do |io|
    io.puts '%{'
    io.puts '%}'
    io.puts 'struct unicode_table_entry;'
    io.puts '%%'
  ...
  unfold_indices.each do |to_codes_len, items|
    unfold_table_source = IO.popen([*GPERF_COMMAND, '-H', "unicode_unfold#{to_codes_len}_table_hash", '-Q', "unicode_unfold#{to_codes_len}_table_pool", '-N', "unicode_unfold#{to_codes_len}_table_lookup", err: :err], 'w+') do |io|
      io.puts '%{'
      io.puts '%}'
      io.puts 'struct unicode_table_entry;'
      io.puts '%%'
      items.each do |(to_codes, unfold_index)|
        ...
        io.puts "%-20s %d" % ["\"#{to_codes_str}\",", unfold_index]
      end
      io.puts '%%'
      io.close_write
...
def gen_sb(case_map, enc, prefix)
  ...
  codes.each_with_index do |enc_code, index|
    ...
    begin
      ...
      if fold_result.length > 1
        raise "Unexpected fold result for code #{unicode_code} (#{enc_code} in encoding #{enc}): #{fold_result.inspect}"
      end
  ...
  diff_map.each do |diff, range_set|
    ...
    puts "    unfold_items[unfold_count].unfolded_code = (uint32_t)(code #{diff < 0 ? '-' : '+'} #{diff.abs});"
    puts "    unfold_count++;"
    puts "  }"
  end
...
def gen_mb2(case_map, enc, prefix)
  ...
  diff_map.each do |diff, range_set|
    ...
    range_set.each_range do |r|
      if r.begin == r.end
        cond << "code == 0x%02X" % (r.begin + diff)
      else
        cond << ("code >= 0x%02X && code <= 0x%02X" % [r.begin + diff, r.end + diff])
      end
    end
    puts "  if (#{cond.join(' || ')}) {"
    puts "    if (folded_codes != NULL) {"
    puts "      folded_codes[0] = (uint32_t)(code #{diff < 0 ? '+' : '-'} #{diff.abs});"
    puts "    }"
    puts "    return 1;"
    puts "  }"
  end
